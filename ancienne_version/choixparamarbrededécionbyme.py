...
# =========================
# arbre de decision code a la main
# =========================
...
# =========================
# test de plusieurs hyperparametres pour l arbre maison
# =========================

from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, confusion_matrix, classification_report

# liste des hyperparametres a tester
tests = [
    (5, 15),
    (10, 50),
    (20, 25),
...
for max_depth, min_samples_split in tests:
    print("=" * 70)
    print("Test avec max_depth =", max_depth, "et min_samples_split =", min_samples_split)
    ...
    # entrainement sur tout le train
    arbre_maison.fit(X_train_np, y_train_np)

    # prediction sur tout le dev
    y_dev_pred = arbre_maison.predict(X_dev_np)

    # calcul des metriques
    accuracy = accuracy_score(y_dev_np, y_dev_pred)

    precision_high = precision_score(y_dev_np, y_dev_pred, pos_label=1)
    recall_high = recall_score(y_dev_np, y_dev_pred, pos_label=1)
    f1_high = f1_score(y_dev_np, y_dev_pred, pos_label=1)

    precision_low = precision_score(y_dev_np, y_dev_pred, pos_label=0)
    recall_low = recall_score(y_dev_np, y_dev_pred, pos_label=0)
    f1_low = f1_score(y_dev_np, y_dev_pred, pos_label=0)

    matrice = confusion_matrix(y_dev_np, y_dev_pred)

    # dans la matrice :
    # [[vrais_low_predits_low, vrais_low_predits_high],
    #  [vrais_high_predits_low, vrais_high_predits_high]]

    vrais_low_bien_predits = matrice[0][0]
    faux_high_pour_low = matrice[0][1]

    faux_low_pour_high = matrice[1][0]
    vrais_high_bien_predits = matrice[1][1]
    ...
    resultats.append({
        ...
        "f1_low": f1_low,
        "vrais_low_bien_predits": vrais_low_bien_predits,
        "faux_low_pour_high": faux_low_pour_high,
        "vrais_high_bien_predits": vrais_high_bien_predits
    })

# transformer les resultats en dataframe pour comparer facilement
...
# tri par accuracy
print("=" * 70)
print("Meilleurs resultats selon accuracy :")
print(resultats_df.sort_values(by="accuracy", ascending=False))

# tri par f1_low pour voir si le modele reconnait mieux la classe Low
print("=" * 70)
print("Meilleurs resultats selon F1 Low :")
print(resultats_df.sort_values(by="f1_low", ascending=False))

# tri par recall_low pour voir si le modele detecte plus de Low
print("=" * 70)
print("Meilleurs resultats selon Recall Low :")
print(resultats_df.sort_values(by="recall_low", ascending=False))