...
def train_final_model(splits, best_parameters: dict) -> Pipeline:
   ...
   #  ajoue pour enregistrer les resultat et ajouter au systeme commen et comparer au autre modeles 
    
    y_true01 = (splits["y_test"] == "High").astype(int)
    y_pred01 = (test_predictions == "High").astype(int)
    enregistrer_resultats("Random Forest", y_true01, y_pred01, test_probabilities)
    os.makedirs("artefacts/modeles", exist_ok=True)
    joblib.dump(model, "artefacts/modeles/Random Forest.pkl")
